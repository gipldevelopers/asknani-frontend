...
export default function DaycareListingsPage() {
  ...
  const [sortBy, setSortBy] = useState(searchParams.get("sort") || "relevance"); // relevance | price_asc | price_desc | rating
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        ...
        {!isFiltering && featuredDaycares.length > 0 && (
      ...
      <div className="relative h-40 w-full overflow-hidden">
        ...
        <button
          ...
          className={`absolute right-3 top-3 rounded-full bg-white/90 p-2 shadow ${
            favs.has(d.id) ? "text-red-500" : "text-gray-500"
          }`}
        >
      ...
      <div className="flex shrink-0 flex-col items-end gap-2">
        <button
          ...
          className={`rounded-full border p-2 hover:bg-gray-50 ${
            favs.has(d.id) ? "text-red-500" : "text-gray-600"
          }`}
        >
          <Heart className="h-4 w-4" />
        </button>
  ...
  return (
    <div className="mx-auto mt-10 max-w-lg rounded-2xl border border-dashed border-gray-300 bg-white p-8 text-center">
      ...
      <p className="mt-1 text-sm text-gray-600">
        Try changing or clearing filters to see more options.
      </p>
      <button
        onClick={onReset}
